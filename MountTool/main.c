...
static int is_our_mount(const char *source) {
    struct statfs *mounts = NULL;
    int count = getmntinfo(&mounts, MNT_NOWAIT);
    for (int i = 0; i < count; i++) {
        if (strcmp(mounts[i].f_mntonname, kMountPoint) == 0 &&
            strcmp(mounts[i].f_fstypename, "bindfs") == 0) {
...
int main(int argc, char **argv) {
    ...
    if (strcmp(argv[1], "status") == 0) {
        return is_our_mount(argc == 3 ? argv[2] : NULL) ? 0 : 1;
    }
    if (strcmp(argv[1], "unmount") == 0) {
        if (!is_our_mount(NULL)) return 0;
    ...
    if (strcmp(argv[1], "mount") == 0 && argc == 3) {
        ...
        if (is_our_mount(source)) return 0;
        if (is_our_mount(NULL) && unmount(kMountPoint, 0) != 0) {
            fprintf(stderr, "replace existing bindfs: %s\n", strerror(errno));
            return errno ? errno : 1;
        }
